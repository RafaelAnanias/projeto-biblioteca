// Define a estrutura de um objeto Livro. Usar uma interface garante
// que todos os objetos de livro tenham as mesmas propriedades.
interface Livro {
    isbn: string;
    titulo: string;
    autor: string;
...
class BibliotecaApp {
    // O Map é a estrutura de dados central. A chave é o ISBN (string),
    // e o valor é o objeto Livro. O Map é ideal para buscas rápidas pela chave.
    private acervo: Map<string, Livro>;
    
    // Referências aos elementos do HTML para manipulação.
    private formCadastro: HTMLFormElement;
    private formBusca: HTMLFormElement;
    private listaLivrosDiv: HTMLDivElement;
    private btnListarTodos: HTMLButtonElement;

    constructor() {
        // Inicializa o acervo como um novo Map vazio.
        this.acervo = new Map<string, Livro>();

        // Associa as propriedades da classe com os elementos do DOM.
        this.formCadastro = document.getElementById('form-cadastro') as HTMLFormElement;
        this.formBusca = document.getElementById('form-busca') as HTMLFormElement;
        this.listaLivrosDiv = document.getElementById('lista-livros') as HTMLDivElement;
        this.btnListarTodos = document.getElementById('btn-listar-todos') as HTMLButtonElement;
    ...
    private adicionarLivro(): void {
        ...
        const novoLivro: Livro = { isbn, titulo, autor, ano, disponivel: true };
    ...
    private buscarLivros(): void {
        const termo = (document.getElementById('termo-busca') as HTMLInputElement).value.toLowerCase();
        const tipo = (document.getElementById('tipo-busca') as HTMLSelectElement).value;
    ...
    private removerLivro(isbn: string): void {
    ...
    private alternarDisponibilidade(isbn: string): void {
    ...
    private renderizarLista(livros: Livro[] = Array.from(this.acervo.values())): void {
    ...
    private carregarDoLocalStorage(): void {
        ...
        if (dadosSalvos) {
            ...
            this.acervo = new Map<string, Livro>(acervoArray);
        }
    }
}
